...
Then("I should click the action button", () => {
    ...
    // Take Id for comparison at fuel page route
...
Then("I should be redirected to the add fuel page", () => {
    cy.get("@vehicle_id").then((vehicle_id) => {
        cy.url().should("include", `/fuel/add?vehicle_id=${vehicle_id}`)
    })
})
